...
<section>
                            ...
                            <div class="row">
                                ...
                                <div class="col-lg-6">
                                    <div class="form-group mb-4">
                                        ...
                                        <select name="country" class="form-select country" id="country" required></select>
    ...
    <div class="form-group mb-4">
        ...
        <select name="secondary_country" id="secondary_country" class="form-select"></select>
...
<script>
    $(document).ready(function() {
        // List of phone input fields to initialize
        var phoneFields = [{
                selector: "#mobile_number",
                dbValue: "{{ $data['mobile_number'] ?? '' }}"
            },
            {
                selector: "#secondary_mobile_number",
                dbValue: "{{ $data['secondary_mobile_number'] ?? '' }}"
            },
            {
                selector: "#home_telephone",
                dbValue: "{{ $data['home_telephone'] ?? '' }}"
            },
            {
                selector: "#work_number",
                dbValue: "{{ $data['work_number'] ?? '' }}"
            },
            {
                selector: "#secondary_home_telephone",
                dbValue: "{{ $data['secondary_home_telephone'] ?? '' }}"
            },
            {
                selector: "#secondary_work_number",
                dbValue: "{{ $data['secondary_work_number'] ?? '' }}"
            }
        ];

        var itiObjects = {}; // store intlTelInput instances

        phoneFields.forEach(function(field) {
            var input = document.querySelector(field.selector);
            if (!input) return;
            ...
            // Pre-populate if DB value exists
            if (field.dbValue) {
                iti.setNumber(field.dbValue); // e.g. +8801729345196
            }
        ...
        $("form").on("submit", function(e) {
            ...
            phoneFields.forEach(function(field) {
                var input = document.querySelector(field.selector);
                var iti = itiObjects[field.selector];
                var errorSpan = $(input).siblings(".text-danger");

                // ✅ FIXED: use .val(), not .value
                if ($(input).val().trim() === "") return;

                if (!iti.isValidNumber()) {
                    errorSpan.text("Please enter a valid number.");
                    input.focus();
                    hasError = true;
                } else {
                    errorSpan.text("");
                    // ✅ Replace input value with full international number (+880...)
...
<script>
$(document).ready(function() {
    const mainCountry = @json($data['country'] ?? '').trim();
    const secondaryCountry = @json($data['secondary_country'] ?? '').trim();

    function initCountryField(selector, initialValue) {
        $(selector).select2({
        ...
        $(selector).countrySelector({
            valueType: 'full',
            initialCountry: initialValue || 'United Kingdom'
        });
        ...
        setTimeout(() => {
            ...
            $(selector + ' option').each(function() {
                ...
                const selected = initialValue.toLowerCase();
        ...
        $('form').on('submit', function() {
            const selectedText = $(selector + ' option:selected').text().trim();
            $(selector).html(`<option selected value="${selectedText}">${selectedText}</option>`);
        });
    }

    initCountryField('#country', mainCountry);
    initCountryField('#secondary_country', secondaryCountry);
});
</script>
